...
def load_app_data():
    ...
    try:
        ...
        st.error("❌ Data not found! Please run the preparation notebook first.")
        return None
...
if tab_selection == "🎯 Live Predictor":
    ...
    with col1:
        ...
        teams = app_data['team_statistics']['team_name'].unique()
        leagues = app_data['team_statistics']['league_name'].unique()
    ...
    if st.button("🔮 Generate Prediction", type="primary"):
        if home_team != away_team:
            ...
            # Prediction summary
            st.success(f"**Prediction: {favorite}** (Confidence: {confidence:.1f}%)")
...
# Tab 2: Model Predictions
    ...
    # Add mock predictions for display
    ...
    for _, fixture in high_confidence_fixtures.head(10).iterrows():
        with st.expander(f"⚽ {fixture['home_team']} vs {fixture['away_team']} - {fixture['league_name']}"):
            col1, col2 = st.columns(2)
            ...
            with col2:
                ...
                home_odds = fixture['best_home_odds']
                draw_odds = fixture['best_draw_odds'] 
                away_odds = fixture['best_away_odds']
                ...
                if realistic_odds:
                    ...
                    if fixture['confidence'] >= 0.65:
                        st.success(f"💎 **RECOMMENDED BET**")
                        st.write(f"**Bet:** {predicted_outcome}")
                        st.write(f"**Fair Odds:** {recommended_odds:.2f}")
                        st.write(f"**Confidence:** {fixture['confidence']:.1%}")
                    
                # Bookmaker count
                if not pd.isna(fixture['bookmaker_count']):
